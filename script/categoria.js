...
const Tortas = [
    ...
    {
        nome: "Torta Banoffee com Doce de Leite",
        avaliacao: 4.9,
        preco: "R$ 90,00",
...
//ANA PAULA, EU JURO QUE EU TENTEI SEPARAR POR CATEGORIAS, MAS NÃO DEU😭😭😭😭😭😭, considere meu esforço
function renderizarProdutos(listaDeProdutos) {
    ...
    for (let i = 0; i < listaDeProdutos.length; i++) {
        ...
        section.querySelector(".btn-menos").onclick = function() { alterarQtd(this, -1); };
        section.querySelector(".btn-mais").onclick = function() { alterarQtd(this, 1); };
    }
...
function alterarQtd(botao, mudanca) {
    ...
    if (qtdAtual > 0) localStorage.setItem(nomeDoProduto, qtdAtual);
    else localStorage.removeItem(nomeDoProduto);
...
function atualizarCarrinhoAoVivo() {
    ...
    for (let i = 0; i < todosOsCards.length; i++) {
        ...
        if (elementoPreco && elementoQtd) {
            const precoLimpo = parseFloat(elementoPreco.textContent.replace("R$", "").replace(",", ".").replace(/\s/g, ""));
            const quantidade = parseInt(elementoQtd.textContent);
            if (!isNaN(precoLimpo)) totalGeral += precoLimpo * quantidade;
        }
    }
    const elementoTotal = document.getElementById("valor-total");
    if (elementoTotal) elementoTotal.textContent = "R$ " + totalGeral.toFixed(2).replace(".", ",");
}
...
window.onload = function() {
    ...
    const todasAsListas = [
        ...
        CookiesEBrownies, 
        SobremesasGeladas, 
        CupcakesEMiniBolinhos, 
        LinhaSaudavel, 
        DocesDeBalcao
    ];
    
    for (let i = 0; i < todasAsListas.length; i++) {
        for (let j = 0; j < todasAsListas[i].length; j++) {
            todosOsProdutos.push(todasAsListas[i][j]);
        }
    }
